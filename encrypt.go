...
func Encriptar(inPath, outPath string) {
  ...
  if outPath == "" {
    ...
    // si outPath es directorio existente -> usar basename(inPath)+".kry"
    if ok, err := existsDir(outPath); err == nil && ok {
      outPath = strings.TrimRight(outPath, "/") + "/" + baseName(inPath) + ".kry"
...
func Desencriptar(inPath, outPath string) {
  ...
  if outPath == "" {
    if strings.HasSuffix(inPath, ".kry") {
      outPath = strings.TrimSuffix(inPath, ".kry") + ".dec"
    } else {
      outPath = inPath + ".dec"
    }
  } else {
    if ok, err := existsDir(outPath); err == nil && ok {
      outPath = strings.TrimRight(outPath, "/") + "/" + baseName(inPath) + ".dec"
    } else if len(outPath) > 0 && outPath[len(outPath)-1] == '/' {
      if err := mkdirAll(outPath, 0755); err != nil {
        fmt.Printf("No se pudo crear directorio %s: %v\n", outPath, err)
        return
